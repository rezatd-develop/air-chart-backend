...
export const signIn = async (req, res) => {
    try {
        ...
        res.json(createResponseMessageClass(token, false, translations.loginSuccessful));
    } catch (error) {
        console.error(error);
        res.status(500).json(createResponseMessageClass(null, true, translations.signInFailed));
    }
};
